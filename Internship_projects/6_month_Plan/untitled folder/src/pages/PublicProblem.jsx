...
export default function PublicProblem() {
  ...
  if (loading) return <div style={{padding:'2rem',color:'#555',fontFamily:'JetBrains Mono'}}>loading...</div>
      ...
      <div style={s.container}>
        {!sol || sol.status === 'unsolved' ? (
          <div style={s.noSol}>
            ...
            <p style={s.noSolText}>No solution posted yet for this problem.</p>
          ...
          <div style={s.content}>
            ...
            <div style={s.cols}>
              ...
              <div style={s.col}>
                ...
                {sol.files?.length > 0 && (
                  <div style={s.files}>
                    <div style={s.sectionLabel}>files</div>
                    {sol.files.map((f, i) => (
                      <a key={i} href={f.url} target="_blank" rel="noopener noreferrer" style={s.fileLink}>
                        ↓ {f.name}
                      </a>
                    ))}
                  </div>
...
const s = {
  ...
  fileLink: { display:'block', fontFamily:"'JetBrains Mono',monospace", fontSize:'0.72rem', color:'#00ff88', textDecoration:'none', padding:'0.3rem 0' },
  nav: { display:'flex', justifyContent:'space-between', borderTop:'1px solid #1a1a1a', paddingTop:'1.5rem', marginTop:'1rem' },
